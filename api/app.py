#!/usr/bin/env python3
"""HistoGen API for Vercel — JSON routes only (static UI in public/)."""
...
app = FastAPI(title="HistoGen API", version="1.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
async def fetch_biohub_protein(protein_hash: str, topk_features: int = 8) -> dict:
    ...
    async with httpx.AsyncClient(timeout=120.0, headers=biohub_headers()) as client:
        ...
        if resp.status_code == 401:
            raise HTTPException(
                status_code=502,
                detail="Biohub rejected API key — set BIOHUB_API_KEY in Vercel project env",
            )
...
async def agent_cohort_figures():
    if not has_cohort_figures():
        raise HTTPException(status_code=503, detail="Cohort visual report not bundled")
    return list_figures()
...
async def phoenix_heatmap(case_id: str):
    ...
    path = (
        DATA_DIR
        / "tcga_lung/representative_patients/data_package/per_patient"
        / case_id
