...
from _paths import DEPLOY_ROOT, PATIENTS_JSON, PHOENIX_BUNDLE_ROOT
...
def has_phoenix_bundle(case_id: str) -> bool:
    return (PHOENIX_BUNDLE_ROOT / case_id / "phoenix_summary.json").is_file()
...
def load_representative_patients() -> dict[str, Any]:
    ...
    phoenix_count = sum(1 for patient in patients if patient.get("phoenixAvailable"))
    return {
        "defaultCaseId": default_case_id,
        "count": len(patients),
        "phoenixCount": phoenix_count,
        "source": str(PATIENTS_JSON.relative_to(DEPLOY_ROOT)),
...
def _to_ui_patient(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "caseId": case_id,
        "phoenixAvailable": has_phoenix_bundle(case_id),
        "x": layout["x"] + jx,
        "y": layout["y"] + jy,
        "summary": {
            "caseId": case_id,
